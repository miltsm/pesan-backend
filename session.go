...
func (s *publicSrvr) OnboardWithPassword(ctx context.Context, r *stub.OnboardRequest) (*stub.UserSession, error) {
	...
	refreshToken, refreshExpiry, err = newRefreshToken(newId)
	if err != nil {
		return nil, err
	}
...
// NOTE: refresh
func refreshSession(uid uuid.UUID, token []byte) (*stub.RefreshReply, error) {
	if len(token) == 0 {
	...
	case prsdToken.Valid:
		aTkn, aExp, err := newAccessToken(uid)
		if err != nil {
			return nil, err
		}
